 ...
 * @returns {{ node: import("estree").Node; newText: string }[]}
 */
function walk(node, sourceCode) {
  if (node.type === "BinaryExpression") {
    if (node.operator.startsWith("==")) {
      ...
      return [{ node, newText: `${leftText} ${newOperator} ${rightText}` }];
    }
  ...
  return [{ node, newText: `!(${sourceCode.getText(node)})` }];
}
